...
namespace Robot
{
    ...
    internal static class ConnectionRecieve
    {
        ...
        public const UInt32 totalMsgLenght = 1543766016; //3288596480
        ...
        public static async Task Start()
        {
            ...
            await tcpRead.ConnectAsync(Connection.Host, 30003);
        ...
        private static void FetchValues()
        {
            while (Connection.unityState != Connection.UnityState.offline)
            {
                ...
                try
                {
                    ...
                    if (stream.DataAvailable && stream.Read(packet, 0, packet.Length) != 0)
                    {
                        Connection.tcpConnectionBytesRecieved = (BitConverter.ToUInt32(packet, firstPacketSize - 4));
                        
                        if (BitConverter.ToUInt32(packet, firstPacketSize - 4) >= 1543766016)
                        {
                            Debug.Log("Reading prepare");
                ...
                catch (SocketException e)
                {
                    ...
                    Connection.tcpConnectionBytesRecieved = 0;
        ...
        public static void Read(byte[] packet)
        {
            ...
            bool CheckIfMoving()
            {
                for (int i = 0; i < 6; i++)
                {
                    ...
                    if (BitConverter.ToDouble(packet, 8 + i) == 0) continue;
                    else return true;
                }
                return false;
            }
